...
public class OverscrollListView extends ListView implements OnScrollListener,
		View.OnTouchListener, android.widget.AdapterView.OnItemSelectedListener {
	...
	private boolean bounce = true, rebound = false, recalcV = false,
	 ...
	 * 
	 * @param bouncing
	 *            {@code true } for bouncing effect (this is also the default),
				...
				if (firstVis < nHeaders) {
					...
					if (lastVis == totalItems - nHeaders - nFooters) {
						...
						if (visibleCnt > (nHeaders + nFooters)) {
							measure = getChildAt(visibleCnt - nHeaders
									- nFooters);
							if (measure.getBottom() + velocity > getHeight()
								...
								if (!bounce || Math.abs(velocity) < BREAKSPEED) {
									...
									setSelectionFromTop(lastVis - nHeaders
											- nFooters, getHeight() - divHeight
											- measure.getHeight() - 1);
								}
							}
						} else {
